...
static void	fpf_init_width(t_integer_info *info, intmax_t nb, t_format_spec *fs)
{
	...
	{
		info->space_width = 0;
		info->zero_width = fpf_max(fs->width - info->digits
				- info->prefix_width, 0);
	}
	else
	{
		info->space_width = fpf_max(fs->width - fpf_max(fs->precision,
