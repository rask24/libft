...
/*   Updated: 2023/10/12 20:18:37 by reasuke          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	_print_str(char *str, t_format_spec *fs, t_format_result *fr)
{
	...
	if (fs->width && !(fs->flags & FLAG_MINUS) && !(fs->flags & FLAG_ZERO))
		_print_padding(' ', fs->width - str_len);
	else if (fs->width && !(fs->flags & FLAG_MINUS) && fs->flags & FLAG_ZERO)
		_print_padding('0', fs->width - str_len);
	write(STDOUT_FILENO, str, str_len);
	if (fs->width && fs->flags & FLAG_MINUS)
		_print_padding(' ', fs->width - str_len);
	fr->cnt += _ft_max(fs->width, str_len);
}
